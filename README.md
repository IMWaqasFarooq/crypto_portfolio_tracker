...
|---|---|---|
| State management | `flutter_bloc` (Bloc + Cubit) | Vs. Riverpod: both are solid choices, and Riverpod's compile-safe providers and lower boilerplate for simple cases are genuinely appealing. Bloc's strength here is the explicit Event→State model, which pairs well with `bloc_test`'s "given this event, expect exactly this state sequence" style for testing async flows like pagination, retries, and market data updates. It also integrates cleanly with `go_router`'s stream-based refresh for auth-driven route guards (via a small `Listenable` adapter). `BlocSelector` lets each row subscribe to only its own slice of state, so market-data updates don't cascade into unnecessary rebuilds across the list. Bloc for multi-event flows (Market, Auth), the lighter Cubit for single-purpose state (Portfolio, Settings). |
