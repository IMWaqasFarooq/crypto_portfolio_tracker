...
|---|---|---|
| State management | `flutter_bloc` (Bloc + Cubit) | Over Riverpod: Riverpod's compile-safe providers and lower boilerplate for simple cases are genuinely appealing, but Bloc's Event→State stream makes every state change traceable to a discrete, named intent (`MarketEvent.loadMoreRequested`, `AuthEvent.logoutRequested`), which pays off specifically in `bloc_test`'s `blocTest` pattern — "given this event, expect exactly this state sequence" is a precise, readable spec for async flows like pagination or retry-on-failure, where Riverpod's more direct `ref.read(provider.notifier).method()` style blurs intent from result. It also composes cleanly with `go_router`: the router's `refreshListenable` consumes `AuthBloc`'s state `Stream` directly, no adapter needed. Bloc for multi-event flows (Market, Auth), the lighter Cubit for single-purpose state (Portfolio, Settings) — not everything needs the full event ceremony. |
